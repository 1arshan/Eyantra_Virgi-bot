        ...
        if ( (self._count > 1) and (number_models == 0) ):
            ...
            for i in range(0, number_models):
                ...
                if(lst_name_model[0] == self._attachable_object_prefix):
                    rospy.loginfo( '\033[94m' + ">>> [ur5_2] Vacuum Gripper: Pickable object found {}. Pickable: {}".format(name_model, self._flag_pickable) + '\033[0m')
...
def main():
    rospy.init_node('node_service_server_vacuum_gripper_ur5_2')
    
    ur5_vacuum_gripper = VacuumGripper()
    
    s = rospy.Service('/eyrc/vb/ur5/activate_vacuum_gripper/ur5_2', vacuumGripper, ur5_vacuum_gripper.callback_service_on_request)
