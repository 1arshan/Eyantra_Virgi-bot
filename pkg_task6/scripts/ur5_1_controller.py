...
def handle_bot1():
    ...
    coordinate_matrix = [
        ...
        [
            define_joint_angle_list(161, -119, 12, -83, 20, 0),
            define_joint_angle_list(-124,-90,26,-117,-59,0),    #[01]
            # define_joint_angle_list(-124, -90, 26, -117, -59, 0),
            define_joint_angle_list(56, -114, 2, -70, 123, 0),
        ],
        [
            define_joint_angle_list(-54, -96, 82, -166, -128, 0),
            define_joint_angle_list(-122, -102, 55, 46, 61, 0),
            define_joint_angle_list(55, -83, -82, -17, 123, 0),
        ],
        [
            define_joint_angle_list(-55, -97, 88, 7, 125, 0),
            define_joint_angle_list(-122, -118, 103, 14, 59, 0),
            define_joint_angle_list(52, -84, -83, 173, -130, 0)
        ],
        [
            define_joint_angle_list(-56, -93, 119, -27, 127, 0),
            define_joint_angle_list(-125,-117,135,-17,55,0), #31
            define_joint_angle_list(55,-87,-117,-154,-122,0) #32
    ...
    storing_in_temp_inventory_21=match_box_color_with_index['21']
    #storing_in_temp_inventory_01=match_box_color_with_index['01']
    #storing_in_temp_inventory_20=match_box_color_with_index['20']
    storing_in_temp_inventory_31=match_box_color_with_index['31']
    
    # del match_box_color_with_index['32']
    del match_box_color_with_index['21']
    ...
    w=0
    while first_iteration or len(order_info)>0 or w<9:
        w=w+1
        ...
        if not is_box_in_camera: 
            ...
            if len(order_to_be_procced)==0: #testing
                w=w-1
                print("LIST EMPTY")
                rospy.sleep(4)
                continue
            item_found_in_inventory=0 #testing

            for key, value in match_box_color_with_index.items():
                s =  slice(0,-1)
                item_name = order_to_be_procced[2]
                print(value,key, item_name)
                if item_name == value or item_name == value[s]:
                    box_to_be_pick=key  #key pta chl gyi
                    match_box_color_with_index.pop(key)
                    item_found_in_inventory =1
                    break
                
            if item_found_in_inventory ==0:
                print("BREAKING FROM FIRST WHILE LOOP")
                match_box_color_with_index['21']=storing_in_temp_inventory_21
                box_to_be_pick='21'
                #match_box_color_with_index['31']=storing_in_temp_inventory_31
                #match_box_color_with_index['20']=storing_in_temp_inventory_20
                #for key, value in match_box_color_with_index.items():
    ...
    #---->>>>>>>
            ...
            print("CARTESIAN MOVING")
            ur5.ee_cartesian_translation(0,1.4, 0)
            print("MOVING TO CONVEYER BELT")
